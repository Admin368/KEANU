...
:MainFuction
if "%Keanu.Speech%" equ "Transfer"  (
(
echo/Dim ProSpeak
echo/Set ProSpeak = WScript.CreateObject^("SAPI.SpVoice"^)
echo/ProSpeak.Speak "%Reply%"
) > Keanu.Speech.Engine.reply.Temp.vbs
REM cscript //nologo Keanu.Speech.Engine.Reply.Temp.vbs&del Keanu.Speech.Engine.Reply.Temp.vbs
mosquitto_pub -h 192.168.1.170 -m "Speech" -t Mood\Transfer\State
Echo %Reply%
goto :EOF
)
if "%Keanu.Speech%" equ "ON"  (
(
echo/Dim ProSpeak
echo/Set ProSpeak = WScript.CreateObject^("SAPI.SpVoice"^)
echo/ProSpeak.Speak "%Reply%"
) > Keanu.Speech.Engine.reply.Temp.vbs
REM Start /B Keanu.Speech.Engine.Reply.Temp.vbs
REM del Keanu.Speech.Engine.Reply.Temp.vbs
cscript //nologo Keanu.Speech.Engine.Reply.Temp.vbs&del Keanu.Speech.Engine.Reply.Temp.vbs
Echo %Reply%
goto :EOF
)
if "%Keanu.Speech%" equ "OFF"  (
Echo %Reply%
goto :EOF
)
Echo %Reply%
