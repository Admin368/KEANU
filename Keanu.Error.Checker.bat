...
Echo %keanu.date.time% User:%User% Error Check called by [%Process.Name%] >>%access.logs%
...
Echo %keanu.date.time% User:%User% Error Check called by [%Process.Name%] Result [SUCCESSFUL]>>%access.logs%
)
if %errorlevel% EQU "1" (
Echo %keanu.date.time% User:%User% Error Check called by [%Process.Name%] Result [ERROR FOUND]>>%access.logs%
goto :ErrorLog
)
goto :Done
...
Echo %keanu.date.time% User:%User% [%Process.Name%] ERROR CHECKING ENDED HERE! >>%access.logs%
goto :successCheck
goto :Done
goto :FileStart
